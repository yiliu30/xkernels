...
torch::Tensor marlin_int4_fp8_preprocess(
    ...
    std::optional<torch::Tensor> qzeros_or_none, bool inplace) {
  TORCH_CHECK(qweight.is_cuda(), "qweight is not on GPU");
  TORCH_CHECK(qweight.scalar_type() == torch::kInt,
                  "qweight.dtype != torch.int32");

  const int32_t device_index = qweight.get_device();
  at::cuda::CUDAGuard device_guard(device_index);
  const cudaStream_t stream = at::cuda::getCurrentCUDAStream(device_index);

  torch::Tensor output =
      inplace ? qweight : torch::empty_like(qweight);
  ...
  if (!qzeros_or_none.has_value()) {
    ...
    TORCH_CHECK(qzeros.is_cuda(), "qzeros is not on GPU");
    TORCH_CHECK(qzeros.scalar_type() == torch::kInt,
                    "qweight.dtype != torch.int32");
    TORCH_CHECK(qzeros.get_device() == device_index,
    ...
    marlin_int4_fp8_preprocess_kernel_awq<<<blocks, 32, 0, stream>>>(
        reinterpret_cast<const int32_t*>(qweight.data_ptr()),
        reinterpret_cast<int32_t*>(output.data_ptr()),
        reinterpret_cast<const int32_t*>(qzeros.data_ptr()), size_n,
        size_k, group_size);
  }
