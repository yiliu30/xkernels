...
#include "standalone_torch_utils.h"
...
torch::Tensor marlin_gemm(
  ...
  if (b_type_id == vllm::kFE2M1f.id()) {
    if (b_scales.scalar_type() ==
      ...
      TORCH_CHECK(
          ...
          "bfloat16 (for predecoded scales).");
    }
  ...
  if (a_scales_or_none.has_value()) {
    ...
    a_scales = marlin_empty({0}, torch::kFloat, device);
    TORCH_CHECK(
        a_type.size_bits() != 8,
  ...
  if (c_or_none.has_value()) {
    ...
    c = marlin_empty({size_m, size_n}, c_scalar_type, device);
  }
  ...
  if (use_fp32_reduce) {
    ...
    c_tmp = marlin_empty({max_c_tmp_size}, torch::kFloat, device);
  } else {
    c_tmp = marlin_empty({0}, torch::kFloat, device);
  }
  ...
  if (g_idx_or_none.has_value() && perm_or_none.has_value()) {
    ...
    g_idx = marlin_empty({0}, c_scalar_type, device);
    perm = marlin_empty({0}, c_scalar_type, device);
    a_tmp = marlin_empty({0}, c_scalar_type, device);
  }
  bool has_act_order = g_idx.size(-1) > 0 && perm.size(-1) > 0;

  if (has_act_order) {
    a_tmp = marlin_empty({size_m, size_k}, c_scalar_type, device);
    if (is_k_full) {
  ...
  if (global_scale_or_none.has_value()) {
    ...
    global_scale = marlin_empty({0}, torch::kFloat, device);
    TORCH_CHECK(
  ...
  if (has_bias) {
    ...
    b_bias = marlin_empty({0}, c_scalar_type, device);
  }
  ...
  if (b_zeros_or_none.has_value()) {
    ...
    b_zeros = marlin_empty({0}, c_scalar_type, device);
  }
  ...
  marlin::marlin_mm(
      ...
      at::cuda::getCurrentCUDAStream(device_index), thread_k, thread_n, sms,
